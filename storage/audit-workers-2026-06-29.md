...
#### F-46: shutdown() pre-clears heartbeat+idle timer maps that teardownWorkers() unconditionally re-clears
- File: src/pool.ts:533
- Symbol: shutdown / teardownWorkers
- Category: dedup
- Priority: P2
- Evidence: `shutdown()` drains `heartbeatTimers` (533-537) then `idleTimers` (539-543). Every shutdown() exit path then runs `teardownWorkers` (567/583/602) which re-drains the same two maps (419-429) with no externally-observable state in between — pure redundant work. The retry-timer drain (545-550), queue drain (552-557), and retained-release post (559-563) ARE distinct shutdown-only responsibilities and MUST stay.
- Recommendation: Delete the heartbeat-timer and idle-timer clear loops at pool.ts:533-543; `teardownWorkers` performs both clears on every exit path. Keep the retry-timer/queue/retained-release blocks untouched.
- Risk: LOW — pure removal of redundant clears; `teardownWorkers` is the single authority that empties these maps. Verify the shutdown timer-leak assertions still pass (post-teardown empty state is unchanged).
- Confidence: MEDIUM (score: 23)
- LOC delta: +0 / -11
- Recommended-model: sonnet

#### F-47: Task.worker is a dead field — written once on retention, never read
- File: src/pool.ts:170
- Symbol: Task.worker / createWorker (also types.ts:100)
- Category: loc
- Priority: P2
- Evidence: Grep over src/ + tests/: `task.worker` has exactly one reference — the write at pool.ts:170 inside the retained branch. Never read back; the worker↔task binding the pool consults is `this.pending` (`Map<WorkerLike,Task>`). The other `.worker` hits are `this.workers` (the pool array) and platform/node.ts's `NodeWorkerAdapter.worker` (a different field). Pure dead store; the type member is dead. Same class as the resolved F-34/F-35.
- Recommendation: Delete the `task.worker = worker;` assignment at pool.ts:170 and remove the `worker?: WorkerLike;` member from the `Task` type at types.ts:100.
- Risk: LOW — field is never read; no test references it (grep-confirmed). Behavior identical.
- Confidence: HIGH (score: 23)
- LOC delta: +0 / -2
- Recommended-model: sonnet
